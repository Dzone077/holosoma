...
class JoystickConfig:
    ...
    # logitech
    custom_mode_button: evdev.ecodes = evdev.ecodes.BTN_SOUTH
    rl_gait_button: evdev.ecodes = evdev.ecodes.BTN_WEST
...
class RemoteControlService:
    ...
    def start_custom_mode(self) -> bool:
        ...
        # This is the most reliable way to check, as it avoids issues 
        # with list/tuple types and any extra keys being pressed.
        if self.joystick is not None:
            return self.config.custom_mode_button in self.joystick.active_keys()
        return self.keyboard_start_custom_mode

    # And apply the same to start_rl_gait()
    def start_rl_gait(self) -> bool:
        """Check if gait button is pressed."""
        if self.joystick is not None:
            return self.config.rl_gait_button in self.joystick.active_keys()
